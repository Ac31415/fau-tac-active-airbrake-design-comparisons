...
4. **Dual Execution Modes**:
   - **Simulink Model**: [`models/Rocket_Active_Airbrake_6DOF.slx`](models/Rocket_Active_Airbrake_6DOF.slx) (clean, modular subsystems with scopes and outports).
...
```text
fau-tac-active-airbrake-design-comparisons/
├── models/
│   └── Rocket_Active_Airbrake_6DOF.slx    # Native Simulink 6-DOF Model
├── scripts/
│   ├── init_airbrake_params.m             # Active parameter script (currently loads OpenRocket example config)
│   ├── init_airbrake_params_openrocket_example_simple_rocket.m # Config from Sim 1 of "A simple model rocket" (OpenRocket)
│   ├── init_airbrake_params_fau_tac_old_rocket.m # Template config file for old TAC rocket (to be modified in future)
│   ├── init_airbrake_params_default_gemini.m     # Default 4-inch L-Class sounding rocket config generated by Gemini
...
## Rocket Configuration Scripts

The simulation supports several airframe and motor parameter sets under [`scripts/`](scripts/):

- **[`scripts/init_airbrake_params.m`](scripts/init_airbrake_params.m)**:
  The active master script that the program uses to run the simulation. It currently contains the configurations used in [`scripts/init_airbrake_params_openrocket_example_simple_rocket.m`](scripts/init_airbrake_params_openrocket_example_simple_rocket.m).
- **[`scripts/init_airbrake_params_openrocket_example_simple_rocket.m`](scripts/init_airbrake_params_openrocket_example_simple_rocket.m)**:
  Uses the rocket configuration from **Simulation 1 of the "A simple model rocket" example in OpenRocket** ($25\,\text{mm}$ diameter, $0.425\,\text{m}$ length, $0.0713\,\text{kg}$ liftoff mass, Estes solid motor profile).
- **[`scripts/init_airbrake_params_fau_tac_old_rocket.m`](scripts/init_airbrake_params_fau_tac_old_rocket.m)**:
  The **template config file for the old TAC rocket configs**; this serves as a baseline and will need to be modified in the future with updated vehicle measurements and flight parameters.
- **[`scripts/init_airbrake_params_default_gemini.m`](scripts/init_airbrake_params_default_gemini.m)**:
  Contains the baseline configuration **made default by Gemini** (a high-power $4\,\text{inch} / 102\,\text{mm}$ sounding rocket with Cesaroni/AeroTech L-Class motor, $10.15\,\text{kg}$ liftoff mass, targeting a $3000\,\text{m}$ apogee).
...
And generates 6 publication-quality figures in [`results/`](results/).
